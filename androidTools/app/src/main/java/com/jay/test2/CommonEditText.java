...
public class CommonEditText extends EditText {
    ...
    /**
     * 只有两个参数的构造方法调用下面有3个参数的方法，
     * 传入一个int类型参数方便自己自定义属性，不加这个很多属性不能再XML里面定义
     *
     * @param context
     * @param attrs
     */
    ...
    private void init() {
        ...
        if (funcType == TYPE_CAN_CLEAR) {
            ...
            if (!TextUtils.isEmpty(content)) {    //初始化内容不为空，则不隐藏右侧图标
            ...
            setRightIconVisible(true);   //如果不是清除功能,不隐藏右侧默认图标
        }
